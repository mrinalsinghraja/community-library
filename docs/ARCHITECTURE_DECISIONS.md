...
## ADR-066 — Readers to sixteen, three bands that cover them, and a page of answers

**2026-08-27** · Accepted

### The decision

**Membership runs 5 to 16.** `age_max` is per-library configuration and always
was, so this is a default and a data change rather than new logic: the shipped
default moves from 14 to 16, and the migration moves a library still sitting on
the old default. A library that has deliberately chosen its own range keeps it.

Everything downstream already read the setting, which is why this is a small
change: the joining form's year list is generated by `eligibleBirthYears`, the
joining guide, the home page and the rules page all interpolate `ageMin` and
`ageMax`, and `isEligibleBirthYear` is the only gate. Nothing had to be found
and edited, which is the payoff for never typing the numbers in.

**Three shelf bands, covering the range with no gap:**
...
The bands are a catalogue taxonomy, not a permission — nothing in the borrowing
code has ever checked a reader's age against a book's band, and `AGE_BAND_NOTE`
says so on every screen that prints one. What the old shape got wrong was
arithmetic rather than principle: membership now reaches 16 and the bands
stopped at 14, so a 15-year-old member had no band to belong to and a librarian
cataloguing for them had to pick the nearest lie.

Done with `ALTER TYPE ... RENAME VALUE`, so every existing row keeps its band
with no rewrite and there is no window in which a book has no band. The one real
consequence is that a title banded for 11-year-olds now reads 12–16. That was
checked against production before writing the migration: all four titles there
are `ALL_AGES` and none of them move.

**A questions page at `/faq`,** in the shared navigation list so both shells
carry it for every role, and linked twice from the home page. It answers what
was being answered one message at a time on WhatsApp — who can join, what it
costs, whether a donation is expected, what the library holds about a child, who
can see the photograph, where a reset link goes, what the AI helper is and what
it is not.

Three rules hold it in place, and `tests/unit/faq.test.ts` enforces the first:

1. **No number is typed into it.** Ages, loan periods and limits are read from
   settings; the band list is rendered from `AGE_GROUPS`. A page that quoted the
   numbers would be a second copy of the rules, going stale silently.
2. **No promise the code does not keep.** Each claim is pinned to the behaviour
   it describes.
3. **No reassurance without the mechanism.** "We keep your child's data safe" is
   worth nothing. "The photograph is served only to you and the librarian, and
   checked on every request" is worth something.

### What this does not change

**No age gate appears anywhere.** Not on borrowing, not on the catalogue, not on
the book helper. The band is a signpost and the membership range is a
conversation with a librarian.

**The eligibility check stays generous.** `isEligibleBirthYear` accepts a birth
year when *either* age it could mean falls in range, because the library holds a
year and not a birthday. With the range ending at 16, a child turning 17 is
still accepted — they are 16 until their birthday, and refusing them in January
on the strength of a fact the library chose not to collect is exactly the
behaviour that check exists to prevent.

**The consequences policy.** The first draft of the questions page promised "no
fine for a late book" twice, and `tests/unit/consequences-policy.test.ts` caught
