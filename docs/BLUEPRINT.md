...
**ID generation:** `{settings.copyCodePrefix}-{sequence padded to settings.copyCodePadding}` → `MJCL-0001`, or `LIB-0001` for the next community. Sequence is per-library, allocated inside the creation transaction with a row lock so two librarians adding books at the same desk cannot collide. Codes are never reused, even after archiving — a code is a permanent physical label.
