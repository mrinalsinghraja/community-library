...
/** WhatsApp's glyph. Filled, unlike the stroke icons in `Icon`, so it is drawn here. */
 ...
 *
 * Two callers now want this and neither wants the card around it — the donors
 * page puts it beside the email button, the home page puts it inside the
 * gratitude card. It is a plain `<a>` rather than `ButtonLink` because it
 * leaves the site and so needs `target` and `rel`, and it borrows
 * `buttonClasses` so that leaving the system does not mean leaving the design.
 *
 * Same rule as everywhere else: no number, no button.
 */
export function WhatsAppButton({
  ...
  /** Prefilled, so the sender has nothing to compose. */
  message: string;
  children: ReactNode;
...
export function WhatsAppHelp({
  ...
  heading?: string;
  lead?: string;
  /** What the chat says before the sender types. */
  message?: string;
  cta?: string;
  className?: string;
}) {
